...
static void update_generating_step(void) {
    ...
    if (G.gen_ppos < G.gen_plen) {
        ...
        if (G.gen_ppos >= G.gen_plen) {
            ...
            G.gen_out_count   = 0;
            /* FIX: Append first predicted token — was being lost!
             * gen_last_tok holds the model's prediction from the last
             * prompt token. Without this, 'IBM' shows as 'BM', etc. */
            if (G.gen_last_tok >= 32 && G.gen_last_tok <= 126
                && G.dialog_len < (int)sizeof(G.dialog_buf) - 1) {
                G.dialog_buf[G.dialog_len++] = G.gen_last_tok;
        ...
        uint8_t tok = sgai_next_token(&G.ai, G.gen_last_tok, 0);
        G.gen_last_tok = tok;
        G.gen_out_count++;
